  ...
  return (
    <aside className="rightpanel">
      ...
      <div className="rightpanel__body">
        {/* Cantidades por título (como antes) */}
          ...
          {entries.length === 0 ? (
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gap: 6 }}>
              {entries.map(([title, count]) => (
                <li key={title} style={{ display: 'flex', justifyContent: 'space-between',
                  border: '1px solid #eee', borderRadius: 8, padding: '8px 10px', background: '#fafafa' }}>
                  <span style={{ fontWeight: 600 }}>{title}</span>
                  <span style={{ minWidth: 28, textAlign: 'center', background: '#111', color: '#fff',
                    borderRadius: 999, padding: '2px 8px' }}>{count}</span>
                </li>
              ))}
            </ul>
          )}
        ...
        <div>
          ...
          {items.length === 0 ? (
            <p className="muted" style={{ marginTop: 6 }}>Sin items con precio configurado.</p>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 8 }}>
              <thead>
                <tr>
                  <th style={{ textAlign: 'left', borderBottom: '1px solid #eee', padding: '6px 4px' }}>Módulo</th>
                  <th style={{ textAlign: 'right', borderBottom: '1px solid #eee', padding: '6px 4px' }}>Cant.</th>
                  <th style={{ textAlign: 'right', borderBottom: '1px solid #eee', padding: '6px 4px' }}>Unit.</th>
                  <th style={{ textAlign: 'right', borderBottom: '1px solid #eee', padding: '6px 4px' }}>Subtotal</th>
                </tr>
              </thead>
              <tbody>
                {items.map((it) => (
